...
Public resources (`skill`, `agent`, `rule`) and nested subagents generate agent-facing names from source frontmatter `name`. By default the system applies `cf-` / `cf-{kit-slug}-`; `prefix_generated_name = false` publishes the frontmatter name as-is. Installation fails before writing files when that generated public name conflicts with another public component in the installing kit or an already registered kit. Agent generation also rejects duplicate public names from existing registrations instead of allowing one generated output to overwrite another.
...
**Resolved resource paths** are stored in `{cf-studio-path}/config/core.toml` under the kit's `[kits.{slug}.resources]` section as `identifier = "resolved_path"` pairs for non-register installs. Persisted paths in `core.toml` are **project-relative only** and must remain inside the current project; absolute-path fallback is not allowed because it makes the installed state non-portable. In `register` mode, `core.toml` stores the authoritative installed kit root plus manifest-root state, and runtime re-derives per-resource locations from the canonical `.cf-studio-kit.toml` instead of persisting per-resource bindings. At runtime, persisted relative bindings are resolved against the project or `{cf-studio-path}` anchor to absolute paths for use by commands. The `cfs info` command outputs resolved resources and variables per kit, enabling agents and scripts to discover resource locations programmatically without storing absolute paths in config.

**Template variable resolution**: Kit files (rules.md, SKILL.md, workflows, etc.) and the execution protocol can reference resource identifiers as `{identifier}` template variables. Installation preserves kit source files byte-for-byte except for accepted file-level diff changes; it does not rewrite template variables inside resources. Runtime commands and `cfs resolve-vars` resolve resource variables from effective installation state first: persisted `core.toml` bindings for copy-mode kits, or canonical manifest resources plus the registered manifest root for `register` mode. `cfs resolve-vars` exposes system variables, per-kit variable sections, and a flat compatibility map; human-facing summaries and `cfs info` report variables per kit rather than as one merged info payload.
...
- `cpt-studio-adr-validation-severity-policy` — validation severity declared in kit and project configuration; a kit's own opinion settles first (entry, per-kind, whole-kit, built-in default) and the project layer is then admitted under a raise/lower rule; a project may raise freely and may lower only what the kit has not marked `locked`, no CLI flag may lower anything, and every lowering and every refusal is named in the report; an unrecognised severity value fails the load because it would disable checking, while an unrecognised key is reported as a warning so a kit written for a newer engine stays installable
...
Its package version is derived from SCM tag metadata at build/install time, so maintainers do not update a checked-in `pyproject.toml` version for releases. Its installed-content version authority is the configured effective source: GitHub releases/tags for GitHub-backed bundles, and local explicit provenance only for local/path modes. Mirror and fork overrides are resolved before authority checks, so reporting never treats the pre-override URL as authoritative.
...
At the bottom is the **Kit layer**. The **Kit Manager** handles kit installation from GitHub repositories or local paths, registration with structured source provenance, file-level diff updates, kit config relocation, and layout migration. Each kit is an independently installable file package. Studio core bundles no kits — during `cfs init`, the user is prompted to install the SDLC kit (`constructorfabric/studio-kit-sdlc`) with `[a]ccept / [d]ecline`. A plugin system for custom hooks and CLI subcommands is planned for p2.
...
- Constraint enforcement from `constraints.toml` (headings scoping, reference rules). For manifest-driven kits, the path to `constraints.toml` is resolved from effective kit resource bindings
- Kit structural validation (`validate-kits`): verify kit directory exists with required files. For manifest-driven kits, resolve paths to constraints, templates, and examples from effective bindings or the registered canonical manifest instead of assuming the default kit directory structure
...
- Manifest-driven installation (see `cpt-studio-fr-core-kit-manifest`): the Kit Manager loads kit sources through a normalized `KitModel`. New kits use canonical `.cf-studio-kit.toml`; legacy `manifest.toml` v1/v2, ADR-0019 component manifests, and `conf.toml + layout` kits remain compatibility inputs. The Kit Manager validates the normalized model, reads all declared resources, prompts for effective destinations or local copy/register mode when required, copies or registers each resource, preserves template variables (`{identifier}`) in installed files, and registers resolved paths, install mode, hashes, provenance, and warnings in `core.toml` for non-register installs. In `register` mode it stores only the authoritative kit root and manifest-root state needed to re-derive resources at runtime. If no manifest-style input is present, it falls back to legacy predefined content directories and files.
- Kit registration: add kit entry to `{cf-studio-path}/config/core.toml` with config output path, source provenance (`github`, `local_path`, or other explicit source type), requested source, effective source after mirror/fork override, version/ref, verification state, and resolved resource paths (`resources` map) only when the install mode persists per-resource bindings
- Version tracking: for GitHub-backed kits, resolve versions through the effective GitHub source's releases/tags; for local/path kits, record local provenance without GitHub update semantics. Kit source, version/ref, identity, and verification state are tracked in `core.toml`; kit-local metadata may also be stored in `{cf-studio-path}/config/kits/{slug}/conf.toml`
    ...
    alt existing project detected
        ...
        alt user accepts
            ...
            alt canonical or legacy manifest input present in kit source
                ...
                Kit Manager->>Kit Manager: preserve {identifier} template variables in kit files
