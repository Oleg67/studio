...
4. [x] - `p1` - **FOR EACH** file - `inst-toc-foreach-file`
   1. [x] - `p1` - Settle this file's heading-depth and section-size bounds: an explicit command-line flag first, else the value configured for its artifact kind, else the engine default - `inst-toc-resolve-options`
   2. [x] - `p1` - Parse existing TOC block between `<!-- toc -->` markers - `inst-toc-parse-existing`
   3. [x] - `p1` - Generate expected TOC from headings - `inst-toc-generate-expected`
   4. [x] - `p1` - Compare existing vs expected: check anchor validity, heading coverage, staleness - `inst-toc-compare`
   5. [x] - `p1` - **IF** mismatch, record error with diff details - `inst-toc-if-mismatch`
   6. [x] - `p1` - Restamp this file's findings at the severity the policy resolves for them, dropping the suppressed and re-partitioning the rest between errors and warnings; record the artifact kind on each finding and collect any lowering the kit refused - `inst-toc-apply-policy`
