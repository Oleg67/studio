...
21. [x] - `p1` - List everything changed since the base commit against the working tree, including untracked files, so newly written work is never absent from the digest, and report the whole listing as unavailable when either query fails rather than presenting a partial list as complete - `inst-change-summary-collect-changed`
