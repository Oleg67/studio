...
3. [x] - `p1` - Validate template headings match constraints heading contract, settling every finding the template phases produce — headings, ID placeholders and reference placeholders alike — through the kit's own declared severity policy and not the project's, for the same reason step 4 does it for examples; a suppressed finding is counted rather than dropped - `inst-validate-headings`
...
- [x] `p1` - **ID**: `cpt-studio-algo-developer-experience-gate-surface`

**Input**: A Studio source tree — `workflows/` plus `skills/`. Either a released kit or local source, with no difference in handling

**Output**: Per workflow, the files reached, the declared stops among them, the emit sites that do not stop, and the halts reached with no menu open; across the tree, how many workflows can reach each menu, which menus fire unconditionally, **how much of the total reachability the ten most-reachable menus carry**, and everything that would make those counts a floor rather than a total — files that could not be read, `LOAD` targets missing from the tree, and menu names with more than one definition

**Steps**:
1. [x] - `p1` - Name the three PDSL actions this counts, in one place, so a schema change is one edit rather than a search - `inst-surface-vocab`
2. [x] - `p1` - Turn text into structure by scanning lines rather than matching patterns: the expression this replaced put `\s*` under `re.M`, and `\s` matches a newline, so the leading run spanned lines and the match went quadratic on whitespace followed by a near-miss — 0.8ms, 3.2ms, 12.5ms, 52.4ms across 500 to 4000 lines. A line scan is provably linear and needs no argument about backtracking. A menu's name runs to the first character the validator would not accept in one — hyphens included, since it permits them and stopping at the first merges `Plan-Approval-Gate` into any real menu called `Plan` — and the corpus writes both `MENU Name` and `MENU Name:`, where keeping the colon files the block under a name no `EMIT_MENU` will ever reference. Read an action through the markdown it is written in: the corpus writes the same action bare and as a list item, and reading only the bare form found 80 of 88 emit sites. Read declarations **only from fenced blocks in the prompt language**, since an unindented sentence beginning with the word for a menu, or naming the emit action without quoting it, is prose and not a gate — measured before adopting, on a corpus where every declaration is already fenced, so nothing counted is lost - `inst-surface-scan`
3. [x] - `p1` - Check the counted actions against the validator's own keyword set at import, failing loudly if one was renamed — otherwise the walk finds nothing and reports it as a clean surface, and nobody questions a number that went down in a project whose goal is to make it go down - `inst-surface-guard`
4. [x] - `p1` - Count a **declared stop** as an `EMIT_MENU` whose emitting sequence ends the turn, once per emit site — the definition fixed so that the task reducing stops and the task measuring them cannot disagree. It is narrower than every `EMIT_MENU`: 101 of 478 emit-site visits in this corpus run straight on, and a gate that auto-proceeds is a ledger entry rather than an interruption. Report the non-stopping sites beside them so the two are visible and never conflated. Report as well how much of the total reachability the ten most-reachable menus carry, because the shape of that distribution changes what a migration costs: if a small set carries most of the surface then typing that set covers most of the behaviour, and if the surface is flat it does not. Report the pair rather than a ratio, so a reader can see how much of the tree the figure is drawn from. It is read by a person rather than by code. Carry everything that would make a count a floor as a field rather than a log line — unreadable files, `LOAD` targets missing from the tree, menu names defined twice — since a walk that quietly skips something under-counts and an under-count is indistinguishable from progress - `inst-surface-outcome`
5. [x] - `p1` - Read `workflows/` and `skills/` to any depth and each as the named subdirectory it is, never as a name matched anywhere under the root. Read the tree in sorted order and follow `LOAD` transitively from each workflow, bounded by the number of files that exist: a closure can visit each at most once, so exceeding that means the cycle guard is not holding, and turning that into a loud failure beats a process that spins — a test bounding only its own wait still leaks a thread that keeps allocating. Collapse `$HOME` out of every path this reports — the tree path, an unreadable file, a warning — through the shared owner `redaction.home_collapsed`, specified under core-infra's `inst-redact-home`, and never a second copy here, since these are shared files a username must not reach: a change to that redaction contract reaches this feature too - `inst-surface-read`
6. [x] - `p1` - Report a `LOAD` target inside the scanned directories that no file provides, and pass over the ones outside them: the kit legitimately loads requirement and specification files this walk has no reason to read, 17 of them, so reporting those would be false alarms on every run — while a target that should be present is a module deleted without its loaders or a name misspelled, and every menu behind it is missing from the count - `inst-surface-missing-loads`
7. [x] - `p1` - Say out loud whatever would make the counts a floor rather than a total — files that could not be read, targets missing from the tree, names with more than one definition — since a caller reading only the numbers would take an under-count for an improvement - `inst-surface-warn`
8. [x] - `p1` - Recognise an emit by the token alone, so a longer word beginning with the keyword is not one and a name is read the way the validator reads it — kept apart from deciding what the sequence does with the emits it has, because folding the two together reads at a cognitive complexity the analyser rejects - `inst-surface-emit`
9. [x] - `p1` - Decide whether an emit stops the turn by reading **the sequence the emit sits in**, not the menu it names: the corpus declares the wait beside the emit, and reading the definition instead scores a stop written as one of the menu's own options, or in its handler for an invalid reply, as though the menu itself had waited — which labelled the plan approval, git-commit-mode and simple-mode gates as menus that interrupt nobody, and scored 315 where reading the emit site scores 377. Scan to the end of the block rather than over a fixed window, since the corpus sets a variable between the emit and the wait, and writes two guarded emits that one wait serves - `inst-surface-stop-definition`
10. [x] - `p1` - Count one workflow's reachable files into the three numbers, treating a `STOP_TURN` as a halt only where no menu is open before it, so that the `STOP_TURN` ending an emit — which is already what makes that emit a stop — is not counted a second time, inflating both numbers from one declaration; 543 of 1875 halts were that double count. Read a halt anywhere in an action rather than only at its start, since the corpus writes 94 at the start of a line and 61 after an `EMIT "..." and`, and count one only in the sections where actions live — a unit's body and its recovery path — because a rule saying the halt action ends a turn states something about halting and halts nothing. Deciding that by whether the line quotes anything was wrong in both directions: it dropped a real halt whose line names a command, and kept rule prose that named none - `inst-surface-count`
11. [x] - `p1` - Walk every workflow deterministically, counting reachable declared stops, emit sites that do not stop, halts reached **with no menu open** — the `STOP_TURN` ending an emit is already what makes it a stop, and counting it again inflates both numbers from one declaration — and which menus fire without a `WHEN`. Refuse a root with no `workflows/` directory rather than return an empty surface, which nothing distinguishes from a tree that declares no gates. Reachability is an **upper bound, not a path trace** — a `LOAD` guarded by `WHEN` is followed anyway, because deciding a guard needs runtime state this does not have — and the number is good for ranking which gates dominate rather than for predicting one run - `inst-surface-walk`
