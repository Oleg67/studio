...
16. [x] - `p1` - Define the per-file link and report types, keeping referenced IDs separate from declared IDs so a changed specification is never reported as tracing to nothing - `inst-change-summary-link-datamodel`
17. [x] - `p1` - Answer read-only multi-record git queries by splitting NUL-delimited output, since a path may legally contain any byte except NUL - `inst-change-summary-git-lines`
