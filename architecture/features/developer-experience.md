...
25. [x] - `p1` - Stream the one git query whose output the repository does not bound, keeping records up to the ceiling and counting the rest as they pass, so the cost the ceiling exists to bound is the cost actually bounded - `inst-change-summary-git-stream`
